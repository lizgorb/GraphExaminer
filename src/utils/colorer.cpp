...
bool Colorer<Graph>::ThreeColor(const Graph& g, map<int, int> &color_map){
	...
	bool valid_coloring =  ThreeColor(g, temp_color_map, "-",
			_index_mapping, color_map);
...
bool Colorer<Graph>::ThreeColor(const Graph& g, map<int, int> color_map, string token,
	...
	if((nV * (nV - 1) / 2) == num_edges(g)){
		...
		valid_coloring = ColorWithSameColor(g, color_map, token, x, y, x_colors, y_colors,
		...
		if (!valid_coloring)
		 {
			valid_coloring = ColorWithDifferentColor(g, color_map, token, x, y, x_colors, y_colors,
					_index_mapping, _final_color_map);
		 }
	}
...
bool Colorer<Graph>::ColorWithSameColor(const Graph& g, map<int, int> color_map,
	...
	// try to color them with the same color
	 ...
	 if(valid_coloring){
		 ...
		 _final_color_map[_index_mapping[x]] = common.front();
		 _final_color_map[_index_mapping[y]] = common.front();
	 }
...
template <typename Graph>
bool Colorer<Graph>::ColorWithDifferentColor(const Graph& g, map<int, int> color_map,
		string token, int x, int y, vector<int>  x_colors, vector<int>  y_colors,
		map<int,int> &_index_mapping, map<int,int> &_final_color_map){
	bool valid_coloring;
	Graph temp;
		...
		for (std::vector<int>::iterator yi = y_colors.begin() ; yi != y_colors.end() && !valid_coloring; ++yi){
			...
			if(y_color != x_color){
			   ...
			   valid_coloring = valid_coloring || ThreeColor(temp, color_map,
					   	   	   	   	   	   	   	   token + "-D("+ std::to_string(x)+","+ std::to_string(y)+")",
												   _index_mapping, _final_color_map);
			   if(debug)
			   {
				   std::cout<< token << ": " << "Valid coloring? " << std::boolalpha << valid_coloring << endl;
